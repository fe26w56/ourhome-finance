 ...
 * 
 * 認証成功後はホーム画面へ遷移します。
 * グループの存在チェックとリダイレクトはProtectedRouteが担当します。
 */
...
export function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // ホーム画面へ遷移
      // グループの存在チェックとリダイレクトはProtectedRouteが担当
      navigate('/');
    }
  };
  ...
  const handleOAuthLogin = async (provider: 'google' | 'apple') => {
    ...
    if (error) {
      setError(error.message || `${provider}でのログインに失敗しました`);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-white px-4">
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-sm p-8 border border-gray-100">
          ...
          <p className="text-gray-600 mb-8">アカウントにログインしてください</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="text-right">
              <Link
                ...
                className="text-sm text-[#73F590] hover:underline"
              >
                パスワードをお忘れですか？
              </Link>
            </div>
          ...
          <div className="mt-6">
            ...
            <div className="mt-6 space-y-3">
              ...
              >
                ...
                <span className="text-gray-700 font-medium">Appleでログイン</span>
              </button>
            </div>
          </div>
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-gray-600">
              アカウントをお持ちでない方は{' '}
              <Link to="/auth/signup" className="text-[#73F590] font-medium hover:underline">
                新規登録
              </Link>
            </p>
          </div>
